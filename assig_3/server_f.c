    ...
    while (1) {
        ...
        switch (systemCallNumber) {
            case 1: // Connect
                ...
                sprintf(buffer, "%d", processIDCounter); // Corrected: Send the client number as the reply
                write(clientFifo, buffer, strlen(buffer) + 1); // Ensure to send null-terminated string
                processIDCounter++;
                break;
            case 2: // Number
                ...
                write(clientFifos[clientCount - 1], buffer, sizeof(buffer)); // Write to the correct client FIFO
                break;
            case 3: // Text
            ...
            case 0: // Exit
                ...
                close(clientFifos[clientCount - 1]); // Close the correct client FIFO
                break;
            case -1: // Shutdown
